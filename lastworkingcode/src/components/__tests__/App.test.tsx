...
describe('App', () => {
  ...
  describe('create mode functionality', () => {
    ...
    it('closes modal without saving when backdrop is clicked in create mode', async () => {
      const user = userEvent.setup();
      render(<App />);
      ...
      // Modal should close and card should NOT appear in TodoBoard (no save)
      expect(screen.queryByTestId('todoTrigger-modal')).not.toBeInTheDocument();

      const todoBoard = screen.getByTestId('todoBoard');
      expect(
        within(todoBoard).queryByDisplayValue('BackdropTestCard')
