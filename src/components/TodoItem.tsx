...
export const TodoItem = ({
  ...
  return (
    <div 
      className={`flex items-center gap-2 w-full ${isBeingEdited ? '' : ''}`}
    >
      ...
      >
        <Icon
          ...
          className={`w-3 h-3 hover:opacity-80 ${isBeingEdited ? '' : ''}`}
      ...
      <button
        ...
        className={`flex-shrink-0 text-gray-700 hover:text-red-600 ${isBeingEdited ? '' : ''} p-1 rounded hover:bg-white/10 cursor-pointer`}
        title="Delete item"
        disabled={isBeingEdited}
      >
        <Icon name="x" className={`w-4 h-4 hover:opacity-80 ${isBeingEdited ? '' : ''}`} alt="Delete item" />
      </button>
    </div>
  );
};
